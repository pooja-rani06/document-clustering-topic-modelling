...
Dataset: [20 Newsgroups](https://scikit-learn.org/stable/datasets/real_world.html#the-20-newsgroups-text-dataset) (via `sklearn.datasets.fetch_20newsgroups`), restricted to 5 categories: `sci.med`, `sci.space`, `comp.graphics`, `rec.sport.hockey`, `talk.politics.guns`.

## What it does

1. **Preprocessing** — lowercases text, strips URLs/emails/file paths, removes stopwords (plus newsgroup-specific boilerplate like `nntp`, `writes`, `article`), and lemmatizes.
2. **TF-IDF vectorization** — 8,000 features, unigrams + bigrams, `min_df=5` / `max_df=0.85` to cut noise.
3. **K-Means clustering** — elbow method to pick `k`, then clusters at `k=5` with `k-means++` init.
4. **LDA topic modeling** — 5 topics fit on a separate count-vectorized matrix.
5. **Dimensionality reduction** — PCA (TruncatedSVD) and t-SNE for 2D visualization of cluster structure.
6. **Evaluation** — silhouette score, cluster purity, NMI, and ARI against the ground-truth labels.
